	...
	if($row)
	{
		...
		if($row)
		{
			...
			while($row=mysql_fetch_assoc($res))
			{
				...
				$glb["templates"]->set_tpl('{$short_text}',reg_text($row["text"],500));
...
{
	global $url, $urlend, $center, $theme_name, $domen_ID, $sys_lng,$headtitle,$templates,$glb;
...
";
		...
		if (mysql_num_rows($result) > 0) 
		{
			...
			$glb["sys_lng"]=$sys_lng;
				...
				if(isset($glb["templates"])) {
					$glb["templates"]->set_tpl('{$center_name}', $h1_value);
